...
class LearningChat:
        ...
        if result == Result.Ban:
            ...
            if (
                ...
                if random.random() < self.config.break_probability:
                    ...
                    log_debug(
                        "群聊学习", f"➤➤达到复读阈值，复读<m>{messages[0].message}</m>")
                    return [self.data.message]
            return None
        else:
            # 回复
            if self.data.is_plain_text and len(self.data.plain_text) <= 1 and not re.search(r'[\u4e00-\u9fff]', self.data.plain_text):
            ...
            answers_cross = []
            for chunk in chunk_list(keywords__in, 500):
                result = await ChatAnswer.filter(
                    context=context,
                    count__gte=answer_count_threshold,
                    keywords__in=chunk,
                )
                answers_cross.extend(result)
    ...
    async def _ban(self, message_id: Optional[int] = None) -> bool:
        ...
        if message_id:
            ...
            try:
                ...
                log_info(
                    "群聊学习", f"待禁用消息<m>{last_reply.message_id}</m>尝试撤回<r>失败</r>")
        else:
            return False
    ...
    async def speak(
        ...
        def group_popularity_cmp(
            ...
            return cmp(
                len(left_messages) /
                left_duration, len(right_messages) / right_duration
            )
        ...
        for group_id, messages in popularity:
            ...
            # 该群每多少秒发一条消息
            avg_interval = (messages[0].time -
                            messages[-1].time) / len(messages)
            ...
            if contexts := await ChatContext.filter(
                ...
                if speak_list:
                    if random.random() < config.speak_poke_probability:
                        ...
                        speak_list.append(MessageSegment(
                            "poke", {"qq": select_user}))
                    return group_id, speak_list
                else:
                    log_debug(
                        "群聊学习", f"主动发言：群<m>{group_id}</m>没有找到符合条件的发言，不发言")
    ...
    async def _check_allow(self, message: Union[ChatMessage, ChatAnswer]) -> bool:
        raw_message = (
            message.message if isinstance(
                message, ChatMessage) else message.messages[0]
        )
